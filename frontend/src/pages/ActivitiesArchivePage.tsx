...
import { Upload, Archive, ExternalLink, Bike, Footprints, Dumbbell, Clock, Route, Loader2, ChevronDown } from "lucide-react";
...
export default function ActivitiesArchivePage() {
  const { data, isLoading, isFetchingNextPage, fetchNextPage, hasNextPage } =
    useInfiniteActivities(20);
  const [sportFilter, setSportFilter] = useState<string>("all");
  const [minDist, setMinDist] = useState("");

  const allActivities: Activity[] = data?.pages.flatMap((p) => p.items) ?? [];
        ...
        <div className="space-y-6">
          <div className="glass-surface rounded-xl p-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              <div className="flex items-center gap-3">
                ...
                {!isLoading && total > 0 && (
                  <Badge className="bg-accent/10 text-accent border-accent/20 text-xs ml-1">
          ...
          <div className="glass-surface rounded-xl p-4 flex flex-wrap items-center gap-3 accent-line-top">
            ...
            {filtered.length !== allActivities.length && (
              <span className="text-xs text-muted-foreground ml-auto">
                {filtered.length} / {allActivities.length} shown
              </span>
            )}
          </div>
          ...
          {isLoading ? (
            ...
            <EmptyState
              ...
              title={allActivities.length === 0 ? "No activities yet" : "No matching activities"}
              description={
                allActivities.length === 0
                  ? "Upload your first GPX file to get started."
                  ...
                  <TableBody>
                    {filtered.map((a) => {
                      ...
                      return (
                        <TableRow
                          key={a.id}
                          className="hover:bg-accent/5 even:bg-muted/30 transition-colors"
                        >
                          <TableCell>
                ...
                {filtered.map((a) => {
                  ...
                  return (
                    <Link
                      ...
                      to={`/activity/${a.id}`}
                      className={cn(
                        "block glass-card rounded-xl p-4 hover:shadow-md border-l-2",
                        colors.border,
                      )}
                    >
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3 min-w-0">
                          <div
                            className={cn(
                              "stat-icon-bg h-9 w-9 rounded-lg shrink-0",
                              colors.iconBg,
                            )}
                          >
