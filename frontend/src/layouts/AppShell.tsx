...
import { LayoutDashboard, Upload, Archive, Shield, Menu, X, LogOut, Bell, BarChart3, UserCircle } from "lucide-react";
    ...
    <div className="flex min-h-screen w-full bg-background">
      ...
      >
        ...
        <nav className="flex-1 px-4 py-5 space-y-1.5 overflow-y-auto">
          {user?.role === "admin" && (
            <>
              <p className="px-3 text-[10px] uppercase tracking-widest text-muted-foreground mb-2">Admin</p>
              {adminLinks.map((link) => {
              ...
              <p className="px-3 text-[10px] uppercase tracking-widest text-muted-foreground mb-2">Athlete</p>
            </>
          )}
          {userLinks.map((link) => {
            const active = location.pathname === link.to;
          ...
          {location.pathname.startsWith("/activity/") && (
            <Link
              ...
              onClick={() => setSidebarOpen(false)}
              className="relative flex items-center gap-3.5 rounded-xl px-4 py-3 text-sm font-medium transition-colors bg-muted text-foreground"
            >
        ...
        <div className="p-5 border-t border-border mt-auto">
          <div className="flex items-center gap-3.5 mb-4 px-2">
            <div className="relative shrink-0">
              <div className="h-10 w-10 rounded-full bg-gradient-to-br from-accent to-accent/70 ring-2 ring-border flex items-center justify-center text-white text-sm font-bold">
